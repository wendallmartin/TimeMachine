...
namespace TheTimeApp.TimeData
{
    ...
    public class TimeData
    {
        ...
        [NonSerialized] 
        private static TimeData TimeDataBase;
        ...
        /// </summary>
        /// <returns></returns>
        public static TimeData Load(string file)
        {
            lock (ReadWrite)
            {
                if (!File.Exists(file))
...
//            else
//                sqlMode.Read = true;
//            SqlHelper = new SqlServerHelper(stringBuilder, sqlMode, SerilizableCommands) {SqlEnabled = AppSettings.SQLEnabled == "true", Port = Convert.ToInt32(AppSettings.SQLPortNumber)};
//            if (string.IsNullOrEmpty(CurrentUserName)) AddUser();
...
//        public void AddUser()
//        {
//            if (AppSettings.SQLEnabled == "true" && SqlHelper == null) SetUpSqlServer();
